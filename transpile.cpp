...
// Due date: 12/3/2024
// Purpose: Transpiles a source file into C++ code by mapping keywords and generating valid C++ syntax.
//-------------------------------------------------------------------------------------------------------------
...
int main() {
    ...
    while (!in.eof()) {
        ...
        if (it == keywords_map.end()) {
            // If the token is not a keyword, write it directly to the output file
            out << token << " ";
            token.clear();
            continue;
        }
